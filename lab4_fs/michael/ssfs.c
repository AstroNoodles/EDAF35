  ...
  st->st_ctime = mod_time;
    ...
    if(di >= 0) {
      ...
      printf("  -- %d > %.*s\n",di,FS_NAME_LEN,de->name);
			...
			st->st_mode = de->mode;
			st->st_mtime = de->mod_time;
      		st->st_ctime = de->mod_time;
			if(de->mod_time > last_modtime){
				last_modtime = de->mod_time;
			}
		}
    else {
...
static int do_read( const char *path, char *buffer, size_t size, off_t offset, struct fuse_file_info *fi )
{
	...
	unsigned short byte_offset = offset % BLOCK_SIZE;
	...
	// now in the right block - now can start reading!


	char bcache[BLOCK_SIZE];
	//reads the first block into the cache
	...
	// and we now go to the remainder of the blocks after the first one is read
	size -= rsize;
	bid = bmap[bid];

	// while there are still blocks remaining to be read, read the next block from the list
	...
	// read the remaining blocks if there is a remainder from size / BLOCK_SIZE
	...
	return rsize;  
  // how much did we read? - Return all read from all blocks 
}
...
static int do_write( const char *path, const char *buffer, size_t size, off_t offset, struct fuse_file_info *fi )
{
	...
	if(offset + size > de->size_bytes) {
			...
			// set last modification time to current time
			de->mod_time = time( NULL );
...
static int do_truncate(const char *path, off_t offset) {
  printf("--> Trying to truncate %s, %ld\n", path, offset);
  	// locate file
	// skip the "/" in the begining
	const char* fn = &path[1];
	// let's figure out the dir entry for the path
	load_directory();
  	...
  	if(di<0) {
		...
		// load the block map
		load_blockmap();
		// first figure out where the trunc starts (offset in blocks)
		...
		// find the given directory and indicate new size and offset
		dir_entry *de = index2dir_entry(di);
    	de->size_bytes = offset;
		...
		// if the given file doesn't exist, then truncating it creates the file
		if (cur == EOF_BLOCK && block_offset > 0) {
			cur = alloc_block();
			de->first_block = cur;
		}
		// if additional space needs to be allocated to grow file, we add it here
		...
		if (cur != EOF_BLOCK) cur = bmap[cur];

		// loop to free the blocks of the file, looping until we free the blocks attached to current one
...
static int do_unlink(const char *path) {
  printf("--> Trying to remove %s\n", path);
	...
	if(to_move && to_move_id != file_id){
		...
		rm_file_de->mod_time = mv_file_de->mod_time;
...
static int do_rename(const char *opath, const char *npath) {
  printf("--> Trying to rename %s to %s\n", opath, npath);
	...
	// TODO REMOVE TARGET FILE IF IT EXISTS
...
static int do_create(const char *path, mode_t m, struct fuse_file_info *ffi) {
  ...
  load_directory();
	...
	de->mod_time = time( NULL ); // SET modtime TO CREATION TIME
