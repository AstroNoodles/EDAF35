...
//	node_t** last=&block_head;
	while(temp!=NULL){
		if(temp->is_free && (block_data_size(temp)) >= size){
			return temp;
		}
		temp=temp->next;
	}

	block* nblock = new_block(size);
	return nblock;
}
...
void *realloc(void *ptr, size_t size) {
  ...
  struct block* block_ptr = data_to_block(ptr);
...
{
	size_t size = 0;
    if (__builtin_umull_overflow(nitems, elem_size, &size)) {
    // overflow occured
        errno = ENOMEM;
        return NULL;
  }
