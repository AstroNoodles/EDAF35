...
pub const struct_block_s = extern struct {
    is_free: bool = @import("std").mem.zeroes(bool),
    next: [*c]struct_block_s = @import("std").mem.zeroes([*c]struct_block_s),
};
pub const block = struct_block_s;
pub var first: [*c]block = null;
pub fn aligned_size(arg_sz: usize) callconv(.C) usize {
    var sz = arg_sz;
    return ((sz +% @alignOf(max_align_t)) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1))))) & ~(@alignOf(max_align_t) -% @as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))));
}
pub fn block_total_size(arg_pb: [*c]block) callconv(.C) usize {
    var pb = arg_pb;
    if (pb == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return 0 else return @as(usize, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(@as([*c]u8, @ptrCast(@alignCast(pb.*.next)))) -% @intFromPtr(@as([*c]u8, @ptrCast(@alignCast(pb)))))), @sizeOf(u8))));
    return @import("std").mem.zeroes(usize);
}
pub fn block_data_size(arg_pb: [*c]block) callconv(.C) usize {
    var pb = arg_pb;
    if (pb == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return 0;
    return block_total_size(pb) -% @sizeOf(block);
}
pub fn block_to_data(arg_pb: [*c]block) callconv(.C) ?*anyopaque {
    var pb = arg_pb;
    if (pb == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    return @as(?*anyopaque, @ptrCast(pb + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1)))))));
}
pub fn data_to_block(arg_p: ?*anyopaque) callconv(.C) [*c]block {
    var p = arg_p;
    if (p == @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)))) return null;
    return @as([*c]block, @ptrCast(@alignCast(p))) - @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1)))));
}
pub export fn used_size() usize {
    if (first == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return 0;
    var s: usize = 0;
    var last_addr: ?*anyopaque = sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0)))));
    {
        var pb: [*c]block = first;
        while (pb != @as([*c]block, @ptrCast(@alignCast(last_addr)))) : (pb = pb.*.next) {
            if (!pb.*.is_free) {
...
pub export fn unused_size() usize {
    if (first == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return 0;
    var s: usize = 0;
    var last_addr: ?*anyopaque = sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0)))));
    {
...
pub export fn display_list() void {
    var us: usize = 0;
    var es: usize = 0;
    var last_addr: ?*anyopaque = sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0)))));
    _ = fprintf(stderr, "sbrk(0) = %p\n", last_addr);
    _ = fprintf(stderr, "align: %u, meta: %u\n", @as(c_uint, @bitCast(@as(c_uint, @truncate(@alignOf(max_align_t))))), @as(c_uint, @bitCast(@as(c_uint, @truncate(@sizeOf(block))))));
    if (first != @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        {
            var pb: [*c]block = first;
            while (pb != @as([*c]block, @ptrCast(@alignCast(last_addr)))) : (pb = pb.*.next) {
                _ = fprintf(stderr, "(block @ %p) %p:%8ld [%1d]\n", pb, pb + @as(usize, @bitCast(@as(isize, @intCast(@as(c_int, 1))))), block_data_size(pb), @as(c_int, @intFromBool(pb.*.is_free)));
                if (!pb.*.is_free) {
    ...
    _ = fprintf(stderr, "---- used: %zu unused: %zu ----\n", us, es);
    _ = fflush(stderr);
}
pub export fn reset() void {
    if (first != @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        var crtp: [*c]u8 = @as([*c]u8, @ptrCast(@alignCast(sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0))))))));
        var pend: [*c]u8 = @as([*c]u8, @ptrCast(@alignCast(sbrk(@divExact(@as(c_long, @bitCast(@intFromPtr(@as([*c]u8, @ptrCast(@alignCast(first)))) -% @intFromPtr(crtp))), @sizeOf(u8))))));
        if (@as(isize, @intCast(@intFromPtr(pend))) == @as(isize, @bitCast(@as(c_long, -@as(c_int, 1))))) {
            _ = fprintf(stderr, "BRK error!\n");
        }
        first = null;
    }
}
pub fn new_block(arg_size: usize) callconv(.C) [*c]block {
    var size = arg_size;
    var toalloc: usize = aligned_size(size +% @sizeOf(block));
    var nb: [*c]block = @as([*c]block, @ptrCast(@alignCast(sbrk(@as(isize, @bitCast(toalloc))))));
    if (@as(isize, @intCast(@intFromPtr(nb))) == @as(isize, @bitCast(@as(c_long, -@as(c_int, 1))))) {
        __errno_location().* = 12;
        return null;
    }
    nb.*.is_free = @as(c_int, 0) != 0;
    nb.*.next = @as([*c]struct_block_s, @ptrCast(@alignCast(sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0))))))));
    return nb;
}
pub fn find_block(arg_ptr: ?*anyopaque) callconv(.C) [*c]block {
    var ptr = arg_ptr;
    if (first == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) return null;
    var tofind: [*c]block = data_to_block(ptr);
    var last_addr: ?*anyopaque = sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0)))));
    {
        var pb: [*c]block = first;
        while (pb != @as([*c]block, @ptrCast(@alignCast(last_addr)))) : (pb = pb.*.next) {
...
pub fn split_block(arg_pb: [*c]block, arg_size: usize) callconv(.C) isize {
    var pb = arg_pb;
    var size = arg_size;
    var rest: isize = @as(isize, @bitCast(block_data_size(pb) -% aligned_size(size +% @sizeOf(block))));
    if (rest >= @as(isize, @bitCast(@as(c_long, @as(c_int, 0))))) {
        var pn: [*c]block = @as([*c]block, @ptrCast(@alignCast(@as([*c]u8, @ptrCast(@alignCast(pb))) + aligned_size(size +% @sizeOf(block)))));
        pn.*.next = pb.*.next;
        pn.*.is_free = @as(c_int, 1) != 0;
        pb.*.next = pn;
    }
    return rest;
}
pub fn merge_blocks(arg_pb: [*c]block) callconv(.C) void {
    var pb = arg_pb;
    var last_addr: ?*anyopaque = sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0)))));
    if ((pb == @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) or (pb == @as([*c]block, @ptrCast(@alignCast(last_addr))))) return;
    while (pb.*.next != @as([*c]struct_block_s, @ptrCast(@alignCast(last_addr)))) {
        var pn: [*c]block = pb.*.next;
        if ((@as(c_int, @intFromBool(pb.*.is_free)) != 0) and (@as(c_int, @intFromBool(pn.*.is_free)) != 0)) {
            pb.*.next = pn.*.next;
        } else {
            pb = pn;
        }
...
pub export fn total_size_deallocate(arg_starting: [*c]block) usize {
    var starting = arg_starting;
    var total_block_size: usize = undefined;
    {
        var start: [*c]block = starting;
        while (start != @as([*c]block, @ptrCast(@alignCast(sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0))))))))) : (start = start.*.next) {
...
pub export fn free(arg_ptr: ?*anyopaque) void {
    var ptr = arg_ptr;
    if (ptr == @as(?*anyopaque, @ptrFromInt(@as(c_int, 1)))) {
        return;
    }
    if (ptr != null) {
        if (!(first.*.next != null)) return;
        _ = fprintf(stderr, "First has a next\n");
        var found_block_free: [*c]block = find_block(ptr);
        if (found_block_free != null) {
            var last_addr: ?*anyopaque = sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0)))));
            _ = @TypeOf(last_addr);
            found_block_free.*.is_free = @as(c_int, 1) != 0;
        } else return;
    }
}
pub export fn find_free_block(arg_starting: [*c]block, arg_given_size: usize) [*c]block {
    var starting = arg_starting;
    var given_size = arg_given_size;
    while (starting != @as([*c]block, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if ((@as(c_int, @intFromBool(starting.*.is_free)) != 0) and (block_total_size(starting) >= given_size)) {
            return starting;
        }
...
pub export fn malloc(arg_size: c_ulong) ?*anyopaque {
    var size = arg_size;
    if (size == @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) return @as(?*anyopaque, @ptrFromInt(@as(c_int, 1)));
    if (!(first != null)) {
        first = new_block(size);
        _ = fprintf(stderr, "Allocating FIRST block of size %d\n", block_total_size(first));
        return block_to_data(first);
    }
    {
        var start: [*c]block = first;
        while (start != @as([*c]block, @ptrCast(@alignCast(sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0))))))))) : (start = start.*.next) {
            if ((block_total_size(start) == size) and (@as(c_int, @intFromBool(start.*.is_free)) != 0)) {
                _ = fprintf(stderr, "Returning block of size %d\n", start);
                return block_to_data(start);
            } else if ((block_total_size(start) > size) and (@as(c_int, @intFromBool(start.*.is_free)) != 0)) {
                _ = fprintf(stderr, "The block used to have size %d\n", block_total_size(start));
                _ = split_block(start, size);
                return block_to_data(start);
            } else {
                var new_blocks: [*c]block = new_block(size);
                _ = fprintf(stderr, "Allocating block of size %d\n", block_total_size(new_blocks));
                return block_to_data(new_blocks);
            }
        }
    }
    return block_to_data(first);
} // /h/d6/f/mi5048ba-s/edaf35-team18/lab5_mm/ll-mm.c:312:7: warning: TODO implement function '__builtin_umull_overflow' in std.zig.c_builtins
// /h/d6/f/mi5048ba-s/edaf35-team18/lab5_mm/ll-mm.c:309:7: warning: unable to translate function, demoted to extern
pub extern fn calloc(arg_nitems: c_ulong, arg_item_size: c_ulong) ?*anyopaque;
pub export fn realloc(arg_ptr: ?*anyopaque, arg_size: c_ulong) ?*anyopaque {
    var ptr = arg_ptr;
    var size = arg_size;
    if (!(ptr != null)) {
        _ = fprintf(stderr, "Just standard malloc here\n");
        return malloc(size);
    } else if ((ptr != null) and !(size != 0)) {
        free(ptr);
        _ = fprintf(stderr, "There was a pointer but no size\n");
        return @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    } else {
        var found_block: [*c]block = find_block(ptr);
        if (!(found_block != null)) {
            _ = fprintf(stderr, "No block found\n");
            return @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
        }
        var size_block: usize = block_data_size(found_block);
        if (size_block >= size) {
            var rest: isize = split_block(found_block, size);
            _ = @TypeOf(rest);
            return ptr;
        }
        _ = fprintf(stderr, "Need to allocate more blocks. Size is now %d\n", size_block);
        var nextBlock: [*c]block = found_block.*.next;
        if (((nextBlock != @as([*c]block, @ptrCast(@alignCast(sbrk(@as(isize, @bitCast(@as(c_long, @as(c_int, 0))))))))) and (@as(c_int, @intFromBool(nextBlock.*.is_free)) != 0)) and ((block_total_size(found_block) +% block_total_size(nextBlock)) >= (size +% @sizeOf(block)))) {
            var rest: isize = split_block(found_block, size);
            _ = @TypeOf(rest);
            merge_blocks(found_block);
            return ptr;
        }
        var newPtr: ?*anyopaque = malloc(size);
        _ = memcpy(newPtr, ptr, size_block);
        free(ptr);
        return newPtr;
    }
    return @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
...
pub const alignas = @compileError("unable to translate C expr: unexpected token '_Alignas'"); // /auto/cs/EDAF35/zig-linux-x86_64-0.12.0-dev.1108+aeadcb391/lib/include/stdalign.h:18:9
pub const alignof = @compileError("unable to translate C expr: expected '(' instead got 'Eof'"); // /auto/cs/EDAF35/zig-linux-x86_64-0.12.0-dev.1108+aeadcb391/lib/include/stdalign.h:19:9
pub const offsetof = @compileError("unable to translate macro: undefined identifier `__builtin_offsetof`"); // /auto/cs/EDAF35/zig-linux-x86_64-0.12.0-dev.1108+aeadcb391/lib/include/stddef.h:116:9
...
pub const ALIGNMENT = alignof(max_align_t);
pub const META_SIZE = @import("std").zig.c_translation.sizeof(block);
...
pub const block_s = struct_block_s;
